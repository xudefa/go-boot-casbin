# go-boot-casbin 项目开发规范文档

go-boot-casbin 是一个基于 [github.com/xudefa/go-boot](https://github.com/xudefa/go-boot) 的 Casbin 权限管理集成模块。本模块将 Casbin 无缝集成到 go-boot 的 IoC 容器和自动配置体系中，遵循 go-boot 项目的开发规范。
...
- **集成模块**：go-boot-casbin 是 go-boot 的权限管理集成，将 Casbin 作为 RBAC/ABAC 权限引擎实现
- **规范继承**：完全遵循 go-boot 的开发规范、命名约定、代码风格

### 1.2 核心职责

- 将 Casbin Enforcer 注册为 go-boot 容器中的 Bean
- 提供基于 go-boot 自动配置的 Casbin 启动器
- 支持策略加载、模型配置的声明式配置
- 提供 AOP 切面实现方法级权限拦截
...
- **权限引擎**：集成 Casbin（`github.com/casbin/casbin/v2`）
- **职责边界**：仅负责 Casbin 权限集成，不包含其他业务逻辑
- **示例代码**：统一放在 `examples/` 目录，演示 Casbin 集成用法

### 2.2 go-boot-casbin 核心包结构

| 包 | 说明 | 核心类型/函数 |
|---|------|----------|
| `casbin/` | Casbin Enforcer 集成（自动配置、Bean 注册、HTTP 中间件） | `Enforcer`, `NewEnforcer`, `WithModel`, `WithAdapter`, `WithDBAdapter`, `Authorize`, `CasbinAutoConfiguration` |
| `adapter/` | 策略适配器（文件、数据库等存储） | `Adapter`, `DBAdapter`, `NewDBAdapter` |
| `model/` | 模型配置辅助（RBAC、ABAC 模型定义） | `Model`, `RBACModel`, `ABACModel`, `RESTfulModel` |
| `aspect/` | AOP 权限切面（方法级权限拦截） | `PermissionAspect`, `NewPermissionAspect` |

### 2.3 go-boot 核心包参考

go-boot-casbin 依赖 go-boot 的以下核心包：
...
go-boot-casbin 遵循 go-boot 的接口抽象原则，所有集成层通过核心框架中的接口抽象定义，实现运行时互换：

- `core.Container` — IoC 容器
- `boot.AutoConfiguration` — 自动配置
- `boot.Starter` — 启动器生命周期
- `aop.Advice` / `aop.PointCut` — AOP 权限切面
- `config.Config` — 权限配置管理
- `log.Logger` — 日志抽象
...
func init() {
    boot.RegisterAutoConfig(
        ...
        condition.OnProperty("casbin.enabled", "true"),
    )
}
```
